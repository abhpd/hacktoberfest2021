...
# breadth first search implemented for a specific graph 
...
{ 
	...
	//linkedlist to put the final path to the goal node
        LinkedList<Integer> finalqueue = new LinkedList<Integer>(); 
	
	//to count the number of iterations it takes to reach the goal node
        int iteration=1; 
	
	//number of items removed from queue, after visiting if it is not goal node, remove it from queue
        int count=0; 
	...
	void BFS(int s)
	{
		...
		// if an adjacent is not visited enqueue it and add to visited[]
		// if the node is already visited skips it
